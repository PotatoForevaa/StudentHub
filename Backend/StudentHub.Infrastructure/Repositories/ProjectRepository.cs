...
namespace StudentHub.Infrastructure.Repositories
{
    public class ProjectRepository : IProjectRepository
    {
        ...
        public async Task<Result> DeleteAsync(Guid id)
        {
            ...
            if (project == null) return Result.Failure($"Project {id} not found", ErrorType.NotFound);
        ...
        {
            var project = await _dbContext.Projects.FindAsync(id);
            if (project == null) return Result<Project?>.Failure($"Project {id} not found", ErrorType.NotFound);
            return Result<Project?>.Success(project);
        }
